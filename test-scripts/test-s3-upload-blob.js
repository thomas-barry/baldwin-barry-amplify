...
// Test upload with provided URL using Blob (similar to browser File object)
const testWithProvidedUrl = async url => {
  try {
    ...
    // Create a Blob from the file content - this mimics a browser's File object better
    const blob = new Blob([fileContent], { type: fileType });

    console.log(`Created blob with size: ${blob.size} bytes and type: ${blob.type}`);
    console.log('Using fetch API to upload the blob');

    // Upload using fetch API (similar to what the browser would do)
    ...
    console.log(`Upload successful! Status code: ${response.status}`);
